...
struct PaymentMethodView: View {
    ...
    func processPaymentAndBook() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            Task {
                do {
                    if let appId = appointmentId {
                        // Post-appointment payment
                        try await SupabaseDBManager.shared.markAppointmentAsPaid(appointmentId: appId)
                        self.isLoading = false
                        self.dismiss()
                    } else {
                        // Legacy/New booking flow (though we currently skip this in PatientSelectionView)
